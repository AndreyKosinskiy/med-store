...
from .models import Lot, Operation, Store
...
class DocumentForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        excel_book = get_info_from_excel(input_document) # return table witout header
            ...
            for row in excel_book['good_table']:
                ...
                if excel_book['doc_type'] =='-':
                    if is_exists_lot:
                        ...
                        if exists_qty < qty:
                            self.add_error("attachment", f"New qty in attachment biggest then qty of product you have now lot {product_item_batch}: you want substruct {qty} but {exists_qty} you have now.")
                    elif not is_exists_lot:
                        self.add_error("attachment", f"You want substruct {qty} in {product_item_batch} lot, which not exists.")
